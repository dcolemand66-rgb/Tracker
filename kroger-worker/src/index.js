...
async function searchProducts(clientId, clientSecret, term, locationId) {
  ...
  // is exactly what was causing "AUTH-1007: Invalid token on request."
  const token = await getAccessToken(clientId, clientSecret, 'product.compact');
  // TEMP DEBUG: decode the token's own JWT payload so we can see the
  // exact scope/audience actually embedded in what this Worker is
  // sending, to compare directly against the manually-tested token
  // that worked - JWTs aren't encrypted, the payload is just base64.
  try {
    const payloadB64 = token.split('.')[1];
    const payloadJson = atob(payloadB64.replace(/-/g, '+').replace(/_/g, '/'));
    console.log('Worker token payload:', payloadJson);
    globalThis.__lastTokenPayload = payloadJson;
  } catch (e) {
    globalThis.__lastTokenPayload = `decode failed: ${e.message}`;
  }
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // TEMP DEBUG: including the real error message so we can see it
      // directly in the curl response, since both wrangler tail and the
      // dashboard log stream have been unreliable to reach from this
      // environment. Revert to the generic message once everything is
      // confirmed working end to end - a public endpoint shouldn't leak
      // internal error detail long-term.
      return jsonResponse(
        {
          error: 'Kroger lookup failed. Try again in a moment.',
          debug: String(err && err.message ? err.message : err),
          debugTokenPayload: globalThis.__lastTokenPayload || null,
        },
        502
      );
    }
